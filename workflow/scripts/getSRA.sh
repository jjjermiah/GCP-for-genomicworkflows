...
/opt/sratoolkit.3.0.7-ubuntu64/bin/prefetch ${snakemake_wildcards['sra_acc']} -O $SRA_DIR

# remove all files in the SRA_DIR except the SRA files
echo "Removing all files in SRA_DIR except SRA files"
find $SRA_DIR/${snakemake_wildcards['sra_acc']} -type f ! -name "*${snakemake_wildcards['sra_acc']}*" -delete
...
tar -I pigz -cf ${snakemake_output[0]} $SRA_DIR/${snakemake_wildcards['sra_acc']}/
